        ...
        .then(data => {
            if (data.list) {
                ...
                if (forecastList) {
                    ...
                    const curDateTest = '10.03.2025';

                     //const curDayData = groupedByDay[curDateTest];
                    ...
                    curDayData.hourly.forEach(item => {
                        ...
                        for (const [color, range] of Object.entries(colors)) {
                            if (temp >= range[0] && temp <= range[1]) {
                                bg = color;
                                break; // Прерываем цикл, если нашли подходящий диапазон
                            }
                        }

                        let itemHtml = `
                        <div title="${hour}" style="">        
                        ...
                        if(hour == '00:00' || hour == '03:00') {
                            nightForecast.appendChild(li);
                        }          
                        if(hour == '06:00' || hour == '09:00') {
                            morningForecast.appendChild(li);
                        }      
                        if(hour == '12:00' || hour == '15:00') {
                            middayForecast.appendChild(li);
                        }      
                        if(hour == '18:00' || hour == '21:00') {
                            eveningForecast.appendChild(li);
                        }      
                    });
